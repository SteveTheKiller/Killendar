...
// ═══════════════════════════════════════════════════════════
//
// Until this existed, a failure left nothing behind but a Windows Application Error event that
// named KERNELBASE.dll and no line of Killendar. Issue #14 (a crash while typing a start time)
// was diagnosed only because the reporter's Event Viewer happened to hold the .NET Runtime
// entry beside it. This writes that same information to a file the reporter can attach.
//
// The handler deliberately does NOT set e.Handled. Swallowing a dispatcher exception leaves the
// app running in a state nobody reasoned about, and this app writes a calendar file on the
// user's behalf - a save from a half-broken editor is worse than a crash. The log is written,
// then the failure carries on exactly as it did before.
//
...
namespace Killendar
{
    ...
    {
        /// <summary>Beside settings.json under %LOCALAPPDATA%\Killendar, deliberately NOT the
        /// configurable data folder: a crash log belongs on the machine that crashed, and the data
        /// folder can point at a network share that is unreachable at the moment things are going
        /// wrong.</summary>
