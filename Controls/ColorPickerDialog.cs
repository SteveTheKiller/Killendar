...
{
    /// <summary>
    /// KillerUI kit. A small themed RGB color picker ported from KillerNotes (which took it from
    /// KillerPDF): saturation/value square + hue strip, RGB and HTML-hex inputs, a desktop-wide
    /// crosshair eyedropper, and a row of 9 user swatches persisted app-wide ("UserSwatches"
    /// setting). Replace overwrites one slot with the current color; Reset restores defaults.
    /// Opaque RGB only. Used by CategoriesDialog to pick a category's color.
    ///
    /// Built in code rather than XAML because it is kit furniture: it carries its own templates
    /// so it drops into any app in the family without needing entries in that app's Controls.xaml.
    /// </summary>
    internal sealed class ColorPickerDialog : Window
    {
        ...
        /// <summary>The user's 9 swatch slots, app-wide rather than per-Killendar: they are a
        /// preference, not calendar data, so they live in settings.json and not in the .kcal.</summary>
        ...
        private void StoreSaved(List<Color> list) =>
            Settings.Set(SavedKey, string.Join(",", list.Take(SwatchMax).Select(c => $"#{c.R:X2}{c.G:X2}{c.B:X2}")));
        ...
        private TextBox MakeTextBox(double width) => new TextBox
        {
            ...
            // The framework's stock Cut/Copy/Paste menu ignores app-level styling, so these
            // inputs take the app's themed one like every other text box in the family.
            ContextMenu = Application.Current.TryFindResource("TextInputContextMenu") as ContextMenu
        };
        ...
        private static ControlTemplate MakeTextBoxTemplate()
        {
            ...
            foreach (var pair in new[] { (Border.BackgroundProperty, "Background"), (Border.BorderBrushProperty, "BorderBrush"), (Border.BorderThicknessProperty, "BorderThickness") })
                b.SetBinding(pair.Item1, new Binding(pair.Item2) { RelativeSource = new RelativeSource(RelativeSourceMode.TemplatedParent) });
        ...
        private static ControlTemplate MakeBtnTemplate()
        {
            ...
            foreach (var pair in new[] { (Border.BackgroundProperty, "Background"), (Border.BorderBrushProperty, "BorderBrush"), (Border.BorderThicknessProperty, "BorderThickness") })
                bf.SetBinding(pair.Item1, new Binding(pair.Item2) { RelativeSource = new RelativeSource(RelativeSourceMode.TemplatedParent) });
