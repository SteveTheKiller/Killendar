# Adding or Improving a Killendar Translation
...
- **Keep acronyms and proper names as-is:** iCalendar, ICS, Killer Tools.
- **Don't translate the date-format field hint** - it isn't a translated string at all; it's derived automatically from the pattern chosen in `Services/DateFormatManager.cs`, so there's nothing to do there.
