...
namespace Killendar
{
    public partial class MainWindow
    {
        ...
        private void ThemeButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                p.IsOpen = !p.IsOpen;
                // Slides out of the rail rather than just fading, so it reads as coming from the
                // button you pressed. Negative dx = in from the left, toward the calendar.
                if (p.IsOpen && p.Child is UIElement child) Anim.SlideInX(child, -12);
            }
        }
